...
namespace numerical
{
    ...
    void Table::Write(Table::TableType type, const std::string &filename)
    {
        if (!isTableAllocated)
        {
    ...
    void Table::Print(TableType type)
    {
        if (!isTableAllocated)
        {
    ...
    void Table::AddRow(const std::string &newRowName)
    {
    ...
    void Table::AddColumn(const std::string &newColumnName)
    {
    ...
    Table::Table(Table::TableNaming naming)
    {
        this->rowNames = new std::vector<std::string>();
        this->columnNames = new std::vector<std::string>();
        this->rows = 0;
    ...
    void Table::Out(std::ostream &stream, TableType type)
    {
        if (type == TableType::GNUPLOT_TABLE)
        {
            if (naming == TableNaming::NAMED_ROWS_AND_COLUMNS)
